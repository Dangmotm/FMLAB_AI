...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        max_truncate_len = int(self.block_size * 7 / 8)
        min_truncate_len = 4

        if len(document) < min_truncate_len:
            truncate_len = len(document)
        else:
            truncate_len = random.randint(min_truncate_len, min(len(document), max_truncate_len))

        document = document[:truncate_len]
        doc_len = len(document)

        avg_mask_len = doc_len / 4
        masked_len = random.randint(0, doc_len)

        if masked_len == 0:
            mask_start = 0
            mask_end = 0
        else:
            mask_start = random.randint(0, doc_len - masked_len)
            mask_end = mask_start + masked_len

        prefix = document[:mask_start]
        masked_content = document[mask_start:mask_end]
        suffix = document[mask_end:]
        ...
        x = torch.tensor([self.stoi[ch] for ch in input_str], dtype = torch.long)
        y = torch.tensor([self.stoi[ch] for ch in output_str], dtype = torch.long)
